    ...
    fn from(error: Located<Error>) -> Report {
        match *error {
            Error::ExpectedExpression => Report {
                message: "I expected to find an expression, but I did not.",
                region: error.region,
                notes: vec![],
                suggestion: vec![],
            },
            Error::ExpectedExpressionAtom => Report {
                message: "I expected to find an expression atom, but I did not.",
                region: error.region,
                notes: vec![
            ...
            Error::LeftoverSource => Report {
                message: "I thought the code was done, but there was still more text to read.",
                region: error.region,
                notes: vec![],
                suggestion: vec![
            ...
            Error::UnclosedDelimiter => Report {
                message: "I found an open `(`, `[` or `{` that had no matching pair",
                region: error.region,
                notes: vec![],
                suggestion: vec![],
            },
            Error::ExpectedIdentifier => todo!(),
            Error::ExpectedToken(_) => todo!(),
